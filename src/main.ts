...
// ---------- Save ----------
type SaveShape = { credits: number; cargoMax: number; hp: number; sector: string; };
const SAVE_KEY = 'spacetrader_ts_v020';
const readSave = (): SaveShape | null => { try { const raw = localStorage.getItem(SAVE_KEY); return raw ? JSON.parse(raw) as SaveShape : null; } catch { return null; } };
const writeSave = (s: SaveShape) => { try { localStorage.setItem(SAVE_KEY, JSON.stringify(s)); } catch {} };
...
// ---------- App/State ----------
const app = new PIXI.Application({ resizeTo: window, background: '#050812', antialias: true });
document.body.appendChild(app.view as HTMLCanvasElement);

type Asteroid = PIXI.Graphics & { userData: { r: number; hp: number; ore: number } };
type ShipState = { x: number; y: number; vx: number; vy: number; angle: number; thrustLevel: 1 | 2 | 3; hp: number; cargo: number; cargoMax: number; miningRate: number; };
...
const state = {
  ...
  player: { x: -100, y: 60, vx: 0, vy: 0, angle: 0, thrustLevel: 1, hp: 100, cargo: 0, cargoMax: 30, miningRate: 4 } as ShipState,
};
...
// station (spawn relativ zum Spieler, damit sie garantiert „in der Nähe“ ist)
...
// NEU: direkt neben dem Spieler platzieren
station.x = state.player.x + 380;
station.y = state.player.y - 160;
...
window.addEventListener('keydown', (e) => {
  ...
  // Station hervorheben (gelb blinken) – Taste H
  if (e.key.toLowerCase() === 'h' && state.station) {
    const s = state.station;
    (s as any).alpha = 1;
    s.tint = 0xffff66;
    setTimeout(() => { s.tint = 0xffffff; }, 1000);
  }
    ...
    if (state.keys[' ']) {
      ...
      for (const a of state.asteroids) {
        ...
        if (dist2(a, ship) < range2) {
          ...
          // deutlicheres „Flackern“ beim Abbau
          if (Math.random() < 0.25) {
            a.alpha = 0.7;
            setTimeout(() => { a.alpha = 1; }, 60);
          }
      ...
      if (mined > 0) state.player.cargo = Math.min(state.player.cargoMax, state.player.cargo + mined);
    }
  }

  // station leichter Spin; Kamera updaten
...
function updateHUD() {
  ...
  // Zielhinweis + Distanz
  if (state.docked) {
    hud.target.textContent = 'Station';
  } else if (state.station) {
    const d = Math.sqrt(dist2(ship, state.station)) | 0;
    hud.target.textContent = canDock() ? 'Station (E andocken)' : `Station · ${d}`;
  } else {
...
function autoSave() {
  ...
  setTimeout(() => hud.autosave.textContent = 'an', 900);
}

// regelmäßiges Autosave + bei Tabwechsel
